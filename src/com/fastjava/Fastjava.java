...
import com.fastjava.support.html.*;
import com.fastjava.support.servlet.APIHelper;
import com.fastjava.support.servlet.CacheHelper;
import com.fastjava.support.servlet.LogHelper;
import com.fastjava.support.servlet.ModuleHelper;
	...
	private void fastJava(HttpServletRequest req, HttpServletResponse resp) {
		...
		if(methodType.startsWith("mapperHelper")) {
			...
			if("mapperHelper_readPath".equals(methodType)) {	//读取路径
				replaceMap.putAll(new com.fastjava.support.servlet.MapperHelper().readPath(req.getParameter("projectPath")));
			} else if("mapperHelper_createFile".equals(methodType)) {	//生成文件
				new com.fastjava.support.servlet.MapperHelper().createFile(ctx, req.getParameterMap());
				returnJs.append("alert('生成成功，请刷新项目！');");
			}

			//读取表信息
			if(null == req.getParameter("tableInfos")) {
				replaceMap.putAll(new com.fastjava.support.servlet.MapperHelper().makeTableInfo(ctx));
			}
